...
export default function HomeScreen() {
  ...
  const featuredLiveDeals = useMemo(() => {
    const liveFiltered = jvDeals
      ...
      .slice(0, 3);
    // Fallback: if status filter excluded everything, show the first 3 jvDeals so the
    // "3 projects" section always renders the projects that currently exist.
    if (liveFiltered.length === 0 && jvDeals.length > 0) {
      return jvDeals.slice(0, 3);
    }
    return liveFiltered;
  }, [jvDeals]);
