...
import { Activity, ChevronDown, ClipboardList, Cpu, Crosshair, Crown, Gauge, GitBranch, KeyRound, LayoutDashboard, LineChart, Lock, Mail, Megaphone, MessageCircle, Mic, Paperclip, Pin, Radar, Rocket, Search, Send, ShieldCheck, Sparkles, Square, Terminal, Unplug, Upload, UserPlus, Users, X } from 'lucide-react-native';
...
export default function IVXOwnerChatRoute() {
  ...
  const assistantReplyMutation = useMutation<void, Error, { text: string; nonBlocking: boolean; watchdogTraceId?: string | null }>({
    mutationFn: async ({ text, nonBlocking, watchdogTraceId }) => {
      ...
      const watchdogTimer = setTimeout(() => {
        console.log('[IVXOwnerChatRoute] assistant_reply_watchdog_fired — force-clearing typing indicator after 120s.');
      ...
      try {
        ...
        const { value: aiResult, trace: reliabilityTrace } = await executeReliably(
          ...
          // ROOT-CAUSE FIX (2026-06-07): the owner-gated route runs tool-grounded
          // agent-runtime-v2 (15–50s server-side). The old 45s wrapper + 12s
          // per-POST aborted the fetch before the backend answered, producing the
          // BACKEND_POST_FINISHED "Unable to reach IVX Owner AI" no-response. The
          // budget now allows the 58s per-POST ceiling to complete (plus one
          // storm-safe retry) while staying under the watchdog's 180s ceiling.
          { totalTimeoutMs: 95_000, maxAttempts: 2, baseDelayMs: 600, maxDelayMs: 4_000 },
        );
  ...
  const handleCopyTaskLog = useCallback(async () => {
    const tail = displayedMessages.slice(-12).map((m) => `${safeTrim(m.senderLabel) || 'message'}: ${safeTrim(m.text)}`).join('\n');
