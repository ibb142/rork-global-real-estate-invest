-- ============================================================
-- IVX SUPABASE SECURITY HOTFIX
-- ============================================================
-- Run this in Supabase SQL Editor AFTER the full schema has been applied.
-- This migration:
--   1. Drops all permissive "any authenticated" policies on admin/sensitive tables
--   2. Replaces them with owner-only policies via public.ivx_is_owner()
--   3. Fixes user-data policies to be row-scoped (own data only)
--   4. Locks down waitlist read/update/delete to owner-only
--   5. Locks down storage buckets and tightens object policies
--   6. Preserves legitimate public-read and anon-insert policies
-- Safe to re-run. All policy drops use IF EXISTS.
-- ============================================================

-- ============================================================
-- 0. ENSURE ivx_is_owner() EXISTS
-- ============================================================
...
-- ============================================================
-- 1. FIX PROFILES — row-scoped for users, full for owner
-- ============================================================
...
-- ============================================================
-- 2. FIX WALLETS — remove permissive admin_all, keep own-row
-- ============================================================
...
-- ============================================================
-- 3. FIX WALLET_TRANSACTIONS — row-scoped
-- ============================================================
...
-- ============================================================
-- 7. FIX NOTIFICATIONS — row-scoped
-- ============================================================
...
-- ============================================================
-- 8. FIX KYC — row-scoped for user, owner can review
-- ============================================================
...
-- ============================================================
-- 9. FIX PROPERTIES — keep public read, restrict write to owner
-- ============================================================
...
-- ============================================================
-- 10. FIX MARKET_DATA / MARKET_INDEX — keep public read, owner write
-- ============================================================
...
-- ============================================================
-- 11. LOCK DOWN WAITLIST — keep anon/auth insert, owner-only read/update/delete
-- ============================================================
...
-- ============================================================
-- 12. LOCK DOWN JV_DEALS — keep public read for published, owner-only write
-- ============================================================
...
CREATE POLICY "jv_deals_select_published" ON public.jv_deals
  FOR SELECT TO authenticated
  USING (published = true OR public.ivx_is_owner());
...
-- ============================================================
-- 13. LOCK DOWN LANDING TABLES — keep public read/anon insert, owner write
-- ============================================================

-- landing_deals: public read, owner write
...
-- landing_submissions: keep anon/auth insert, owner-only read
...
-- landing_analytics: keep anon/auth insert, owner-only read
...
-- landing_investments: owner-only
...
-- landing_page_config: keep public read, owner write
...
-- landing_deployments: owner-only
...
-- ============================================================
-- 14. LOCK DOWN ANALYTICS TABLES — owner-only
-- ============================================================
...
-- visitor_sessions: keep anon insert, owner-only read
...
-- ============================================================
-- 15. LOCK DOWN SMS TABLES — owner-only
-- ============================================================
...
-- ============================================================
-- 16. LOCK DOWN LENDER TABLES — owner-only
-- ============================================================
...
-- ============================================================
-- 17. LOCK DOWN IMAGE MANAGEMENT — owner-only (keep public read on registry)
-- ============================================================
...
-- ============================================================
-- 18. LOCK DOWN ADMIN/SYSTEM TABLES — owner-only
-- ============================================================
...
-- error_logs: keep anon/auth insert, owner-only read
...
-- app_config: keep public read, owner write
...
-- feature_flags: keep public read, owner write
...
-- ============================================================
-- 19. LOCK DOWN APP TABLES — owner-only where appropriate
-- ============================================================

-- signups: keep anon insert, owner-only read/write
...
-- applications: user can see own, owner sees all
...
-- support_tickets: user can see/create own, owner sees all
...
-- ============================================================
-- 20. LOCK DOWN MARKETING/RETARGETING — owner-only
-- ============================================================
...
-- ============================================================
-- 21. LOCK DOWN db-setup.sql TABLES — owner-only
-- ============================================================
-- These tables were created with "Allow authenticated read/insert/update" for ALL users.
-- Replace with owner-only policies.
...
-- vip_tiers needs public read for the app
DROP POLICY IF EXISTS "owner_only_all" ON public.vip_tiers;
CREATE POLICY "vip_tiers_select_all" ON public.vip_tiers
  FOR SELECT USING (true);
...
-- referrals from full-schema (UUID version): user can see own
DO $$
BEGIN
  ...
  CREATE POLICY "referrals_select_own" ON public.referrals
    FOR SELECT TO authenticated
    USING (auth.uid() = referrer_id OR public.ivx_is_owner());
  CREATE POLICY "referrals_insert_own" ON public.referrals
    FOR INSERT TO authenticated
    WITH CHECK (auth.uid() = referrer_id OR public.ivx_is_owner());
EXCEPTION WHEN undefined_column THEN
  NULL;
END
$$;

-- referral_invites: user can see own
DROP POLICY IF EXISTS "referral_invites_select_auth" ON public.referral_invites;
DROP POLICY IF EXISTS "referral_invites_insert_auth" ON public.referral_invites;

DO $$
BEGIN
  CREATE POLICY "referral_invites_select_own" ON public.referral_invites
    FOR SELECT TO authenticated
    USING (auth.uid() = referrer_id OR public.ivx_is_owner());
  CREATE POLICY "referral_invites_insert_own" ON public.referral_invites
    FOR INSERT TO authenticated
    WITH CHECK (auth.uid() = referrer_id OR public.ivx_is_owner());
EXCEPTION WHEN undefined_column THEN
  NULL;
...
-- deal-photos: public read stays, owner-only insert
CREATE POLICY "deal_photos_owner_insert" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'deal-photos' AND public.ivx_is_owner());

-- investor-intake: anon can still insert (public form), owner reads
CREATE POLICY "investor_intake_anon_insert_v2" ON storage.objects
  FOR INSERT TO anon
  WITH CHECK (bucket_id = 'investor-intake');
...
CREATE POLICY "investor_intake_owner_select" ON storage.objects
  FOR SELECT TO authenticated
  USING (bucket_id = 'investor-intake' AND public.ivx_is_owner());

-- landing-page: public read stays, owner-only write
...
-- chat-uploads: auth user can insert own, owner can read all
...
-- kyc-documents: tighten to own-folder or owner
...
CREATE POLICY "kyc_docs_owner_select" ON storage.objects
  FOR SELECT TO authenticated
  USING (bucket_id = 'kyc-documents' AND public.ivx_is_owner());

-- ============================================================
-- 23. FORCE RLS ON ALL TABLES (defense in depth)
-- ============================================================
DO $$
DECLARE
  tbl RECORD;
