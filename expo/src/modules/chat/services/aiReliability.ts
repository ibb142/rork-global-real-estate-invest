 ...
 * global. Referencing it directly throws `ReferenceError: Property
 * 'DOMException' doesn't exist`, which previously killed the send pipeline
 * after `BACKEND_POST_FINISHED`. We use a tiny Error subclass tagged with
 * `name = 'AbortError'` so all existing `name === 'AbortError'` checks keep
...
export type ReliabilityOptions = {
  ...
  signal?: AbortSignal;
  /** Total budget across all attempts. Default 95_000. */
...
const DEFAULTS = {
  ...
  // agent-runtime-v2 (15–50s server-side). A 45s wrapper budget aborted the
  // in-flight POST before the backend could answer, producing the
  // BACKEND_POST_FINISHED "Unable to reach IVX Owner AI" no-response. The budget
  // now allows the 58s per-POST ceiling to complete (plus one retry) while
  // staying under the watchdog's 180s heavy-audit ceiling.
...
function delay(ms: number, signal: AbortSignal): Promise<void> {
  return new Promise((resolve, reject) => {
    if (signal.aborted) { reject(createAbortError('Aborted')); return; }
    const t = setTimeout(() => { signal.removeEventListener('abort', onAbort); resolve(); }, ms);
    const onAbort = () => { clearTimeout(t); reject(createAbortError('Aborted')); };
...
export async function executeReliably<T>(
  ...
  const timeoutTimer = setTimeout(() => overallController.abort(createAbortError('Total timeout exceeded')), totalTimeoutMs);
