...
function buildIVXOwnerAISystemPrompt(memory: IVXOwnerMemoryState | null, payload?: OwnerAIRequestPayload): string {
  return [
    ...
    'Use tools only when live/current data, schema, logs, code, runtime, or explicit inspection is needed. Normal questions should receive conversational reasoning first.',
...
async function fetchOwnerAIWithHeartbeat(
  ...
  try {
    ...
    const synthesizedBody = final.body == null ? '' : JSON.stringify(final.body);
  ...
  async requestOwnerAI(
    ...
    if (!preflight.ok) {
      ...
      return buildOwnerAuthFailedResponse(input, {
        ...
        body: `No real owner session is signed in on this device (${preflight.reason}), so I ran the owner-session preflight BEFORE any backend POST and stopped. I did NOT send a token that is guaranteed to be rejected (owner_route_auth_401) and I did NOT fall back to the generic /public/chat engine. Your full instruction was preserved (${payload.message.trim().length} characters) and nothing was sent or changed. Open IVX → Auth Diagnostics → Re-authenticate, then resend.`,
      });
    }
    ...
    try {
      // Rolled back 2026-05-27: SSE/heartbeat transport disabled. Owner AI uses
      // simple JSON POST only, same as before 2026-05-26 06:30 AM. The
      // `onProgress` parameter is intentionally ignored here; the watchdog
      // timeout is governed by the JSON request timeout, not heartbeat events.
      void onProgress;
      const result: { endpoint: string; response: Response } = await fetchOwnerAIEndpointWithFallback(accessToken, payload, 'Owner AI request', options?.signal);
