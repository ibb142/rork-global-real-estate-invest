...
/**
 * Visible proof attached to an assistant reply so the chat can show the real
 * live work behind it: which backend route answered, the upstream source
 * (live deal/Supabase data vs. fallback), the model, and the production marker.
 */
export interface ChatMessageMeta {
  ...
  /** Short production commit hash returned by the backend, for the developer-proof line. */
  commitShort?: string;
  /** ISO timestamp the backend produced the answer, for the developer-proof line. */
  backendTimestamp?: string;
}

/**
 * A file attached to a chat message (screenshot, property render, scanned doc
 * photo, budget/appraisal/proforma PDF). After upload it carries a public URL
 * the backend `/public/chat` vision + OCR layers can fetch and analyze.
 */
export interface ChatAttachment {
  ...
  name: string;
  mimeType: string;
  /** Local URI while uploading; replaced by the hosted public URL once ready. */
  localUri: string;
  url: string | null;
