...
import { getIVXOwnerAIEndpoint, getIVXAccessToken } from './ivx-supabase-client';
import {
  sendPublicChatMessage,
  type PublicChatHistoryItem,
  type PublicChatImageInput,
  type PublicChatDocumentInput,
} from './public-chat';
...
interface AIProvider {
  ...
  generate: (prompt: string, context: InvestorContext) => Promise<ProviderGenerateOutput>;
}
...
interface InvestorContext {
  ...
  /** Structured recent turns forwarded to the BLOCK 4 deal-intelligence endpoint. */
  publicChatHistory?: PublicChatHistoryItem[];
  /** Stable session id so the deal-intelligence endpoint keeps conversation continuity. */
  publicChatSessionId?: string;
  /** Image attachments forwarded to the BLOCK 3 visual-intelligence layer. */
  publicChatImages?: PublicChatImageInput[];
  /** Deal-room documents forwarded to the BLOCK 4/5 OCR + analyst layer. */
  publicChatDocuments?: PublicChatDocumentInput[];
}

/** Uploaded attachments forwarded to the deal-intelligence endpoint. */
export interface AIRequestAttachments {
...
/**
 * Backend proof for a single answer so the chat UI can show the real live work:
 * which route answered, the upstream source, the model, and the production marker.
 */
...
type ProviderGenerateOutput = string | ProviderGenerateResult;
...
/**
 * Map recent chat-tab turns into the structured history shape the BLOCK 4
 * `/public/chat` deal-intelligence endpoint expects, so the deployed engine
 * keeps conversation continuity. Support/assistant turns map to `assistant`,
 * everything else to `user`. The welcome bubble and empty turns are dropped.
 */
function buildPublicChatHistory(conversationHistory: ChatMessage[]): PublicChatHistoryItem[] {
  return conversationHistory
    .filter((message) => message.message.trim().length > 0 && message.id !== 'msg-welcome')
    .slice(-MAX_CONTEXT_MESSAGES)
    .map((message) => ({
      role: message.isSupport ? 'assistant' : 'user',
...
function buildRequestKey(
  ...
  return [language, normalizedName, normalizedInterest, normalizedConversation, attachmentSignature ?? '', normalizedMessage].join('|');
}

/**
 * BLOCK 4 deal-intelligence provider. Routes the in-app Chat tab to the deployed
 * `/public/chat` endpoint, which grounds every answer in the business-context
 * engine, visual intelligence, and the deal-scoring / comparison / recommendation
 * layer. This is the highest-priority provider so the existing Chat experience
 * uses the deployed intelligence engine; the other providers remain as failover.
 */
function createProviderPublicChatDealIntelligence(): AIProvider {
  return {
    id: 'public_chat_deal_intelligence',
    name: 'IVX Deal Intelligence (/public/chat)',
    priority: -10,
    status: 'idle',
    failCount: 0,
    lastError: null,
    lastUsed: 0,
    cooldownUntil: 0,
    generate: async (prompt: string, context: InvestorContext): Promise<ProviderGenerateOutput> => {
      const sessionId = context.publicChatSessionId || `investor-chat-${Date.now()}`;
      const payload = await sendPublicChatMessage({
        message: prompt,
        history: context.publicChatHistory ?? [],
        sessionId,
        requestId: `chat-${Date.now()}`,
        images: context.publicChatImages,
        documents: context.publicChatDocuments,
      });

      const answer = typeof payload.answer === 'string' ? payload.answer.trim() : '';
      if (!answer) {
        throw new Error('Deal-intelligence endpoint returned an empty answer');
      }

      console.log('[AIInvestor] Deal-intelligence reply via /public/chat:', {
      ...
      return {
        text: answer,
        meta: {
          source: payload.source === 'chatgpt' ? 'IVX Deal Intelligence (live deal data)' : 'Fallback',
          model: payload.model,
          route: '/public/chat',
          deploymentMarker: payload.deploymentMarker,
          sessionId: payload.sessionId,
          commitShort: payload.commitShort,
          backendTimestamp: payload.timestamp,
        },
      };
    },
  };
...
function createProvider0_IVXOwnerAI(): AIProvider {
  return {
    id: 'ivx_owner_ai',
    name: 'IVX Owner AI Backend',
    priority: 0,
    status: 'idle',
    failCount: 0,
    lastError: null,
    lastUsed: 0,
    cooldownUntil: 0,
    generate: async (prompt: string, _context: InvestorContext): Promise<string> => {
      const endpoint = getIVXOwnerAIEndpoint();
      if (!endpoint) {
        throw new Error('IVX Owner AI endpoint not configured');
      }

      const token = await getIVXAccessToken();
      if (!token) {
        throw new Error('No Supabase session token for IVX Owner AI');
      }
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          requestId: `chat-${Date.now()}`,
          conversationId: 'investor-support-chat',
          message: prompt,
          senderLabel: null,
          mode: 'chat',
      ...
      if (!response.ok) {
        throw new Error(`IVX Owner AI returned HTTP ${response.status}`);
      }

      const data = await response.json() as Record<string, unknown>;
      const answer = typeof data.answer === 'string' ? data.answer : null;
      if (!answer) {
        throw new Error('IVX Owner AI response missing answer field');
      }
...
class AIInvestorService {
  ...
  private publicChatSessionId = `investor-chat-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  async init(): Promise<void> {
    ...
    this.providers = [
      createProviderPublicChatDealIntelligence(),
      createProvider0_IVXOwnerAI(),
  ...
  private async executeProviderChain(userMessage: string, context: InvestorContext): Promise<AIResponseResult> {
    ...
    for (const provider of this.getProvidersToTry()) {
      // If real IVX Owner AI is configured but failed with a backend/network/error,
      // do not silently fall back to canned fake answers. Skip fallback providers
      // so the user sees the actual error instead of a misleading generic response.
      if ((provider.id === 'smart_fallback' || provider.id === 'local_ivx_brain')) {
        const ivxOwnerAiProvider = this.providers.find((p) => p.id === 'ivx_owner_ai');
        if (ivxOwnerAiProvider && ivxOwnerAiProvider.failCount > 0) {
          const isConfigError =
            ivxOwnerAiProvider.lastError?.includes('not configured') ||
            ivxOwnerAiProvider.lastError?.includes('No Supabase session token');
          if (!isConfigError) {
            console.log(`[AIInvestor] Skipping ${provider.name} because real AI provider failed:`, ivxOwnerAiProvider.lastError);
            continue;
          }
        }
      }
      ...
      try {
        ...
        const raw = await withTimeout(provider.generate(userMessage, context), PROVIDER_TIMEOUT_MS, provider.name);
        const text = typeof raw === 'string' ? raw : raw.text;
        const meta = typeof raw === 'string' ? undefined : raw.meta;
        ...
        return {
          ...
          language: context.language,
          failovers,
          meta: meta ?? { route: provider.id, source: provider.name },
        };
      } catch (err) {
    ...
    // If the real IVX Owner AI provider was configured but failed with a non-config
    // error, return the actual error instead of a fake generic response.
    const ivxOwnerAiProvider = this.providers.find((p) => p.id === 'ivx_owner_ai');
    if (ivxOwnerAiProvider && ivxOwnerAiProvider.failCount > 0) {
      const isConfigError =
        ivxOwnerAiProvider.lastError?.includes('not configured') ||
      ...
      if (!isConfigError) {
        return {
          text: `IVX AI Backend error: ${ivxOwnerAiProvider.lastError || 'The AI backend is temporarily unavailable. Please try again.'}`,
          provider: 'ivx_owner_ai_error',
          language: context.language,
          failovers,
        };
      }
  ...
  async generateResponse(
    ...
    attachments?: AIRequestAttachments
    ...
    const publicChatHistory = buildPublicChatHistory(conversationHistory);
    const publicChatImages = attachments?.images ?? [];
    const publicChatDocuments = attachments?.documents ?? [];
    const context: InvestorContext = {
      ...
      conversationSummary,
      publicChatHistory,
      publicChatSessionId: this.publicChatSessionId,
      publicChatImages: publicChatImages.length > 0 ? publicChatImages : undefined,
      publicChatDocuments: publicChatDocuments.length > 0 ? publicChatDocuments : undefined,
    };

    const attachmentSignature = [...publicChatImages.map((image) => image.url), ...publicChatDocuments.map((doc) => doc.url)].join(',');
    const requestKey = buildRequestKey(userMessage, language, investorName, investmentInterest, conversationSummary, attachmentSignature);
