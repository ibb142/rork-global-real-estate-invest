...
function requireAdmin(action: string): { allowed: boolean; userId: string; role: string; error?: string } {
  ...
  return { allowed, userId: userId ?? 'anonymous', role: role ?? 'unknown', error: allowed ? undefined : `Only admin can ${action}. Your role: ${role}` };
}
...
export async function captureDeleteSnapshot(params: {
  ...
  const snapshot: DataSnapshot = {
    ...
    deletedBy: userId ?? 'anonymous',
    deletedByRole: role ?? 'unknown',
