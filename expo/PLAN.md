...
# Runtime fix: "Unexpected text node" crash — 2026-05-29

## Root cause (why prior turns couldn't pin it)
- Static review and a full AST literal scan of every View-like host across `app/`, `components/`, `src/` were **CLEAN** — there is no static text node under a View. The crash is a **data-dependent runtime leak**: a `{expr}` child that only evaluates to a string/number under specific content (the reported case rendered `.`).
- Critical miss in every previous attempt: `babel-preset-expo` uses the **automatic JSX runtime**, so compiled JSX calls `jsx`/`jsxs`/`jsxDEV` (children in `props.children`), NOT `React.createElement`. Guards/patches aimed at `createElement` never intercepted the real render path.

## Fix
- [x] `expo/lib/text-node-guard.ts`: global guard patching `react/jsx-runtime` (`jsx`,`jsxs`), `react/jsx-dev-runtime` (`jsxDEV`, the dev/preview path), and `React.createElement` (classic fallback). Any stray string/number child of a View-like host (`View`, `ScrollView`, `Pressable`, `Touchable*`, `KeyboardAvoidingView`, `SafeAreaView`, `ImageBackground`, Animated wrappers) is auto-wrapped in `<Text>`; whitespace-only strings dropped; offender text + source stack logged once per site for true-source identification. `<Text>` children left untouched.
- [x] Installed at module load in `expo/app/_layout.tsx` (`installTextNodeGuard()`) before any UI renders.
- [x] Diagnostic AST scanner `expo/scripts/scan-text-nodes.mjs` added for ongoing detection.
- [x] `runChecks` (expo) pass; isolated interception simulation proves the reported `.` case is wrapped, multi-child arrays sanitized, and legal `<Text>`/already-valid trees untouched.
