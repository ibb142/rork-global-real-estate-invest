...
export default function EmailComposeScreen() {
  ...
  const canSend = useMemo(() => {
    return to.trim().length > 0 && subject.trim().length > 0;
  }, [to, subject]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        Alert.alert('Sent', 'Your email has been sent successfully via AWS SES.', [
          { text: 'OK', onPress: () => router.back() },
        ]);
      } else {
        Alert.alert('Send Issue', result.error || 'Email saved locally but delivery may be pending. Check SES domain verification.', [
          { text: 'OK', onPress: () => router.back() },
        ]);
      }
    } catch (err: any) {
