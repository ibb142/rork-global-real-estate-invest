...
export default function VideoPresentationScreen() {
  ...
  const handleShareImageWhatsApp = useCallback(async (image: GeneratedImage) => {
    ...
    try {
      ...
      await FileSystem.writeAsStringAsync(shareFileUri, image.base64, { encoding: 'base64' });
      console.log('[VideoPresentation] Share file saved:', shareFileUri);
      const isAvailable = await Sharing.isAvailableAsync();
      if (isAvailable) {
        await Sharing.shareAsync(shareFileUri, { mimeType: image.mimeType });
      } else {
