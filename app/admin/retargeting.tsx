...
export default function RetargetingDashboard() {
  ...
  const dashboardQuery = trpc.engagementIntel.getRetargetingDashboard.useQuery(undefined, { enabled: isAuthenticated });
  const audiencesQuery = trpc.engagementIntel.getAudienceSegments.useQuery(undefined, { enabled: isAuthenticated });
  const pixelsQuery = trpc.engagementIntel.getAdPixelStatus.useQuery(undefined, { enabled: isAuthenticated });
  const seoQuery = trpc.engagementIntel.getSearchDiscoveryData.useQuery(undefined, { enabled: isAuthenticated });
  const triggersQuery = trpc.engagementIntel.getReEngagementTriggers.useQuery(undefined, { enabled: isAuthenticated });
  const utmQuery = trpc.engagementIntel.getUTMAnalytics.useQuery({ period: '30d' }, { enabled: isAuthenticated });
  const scoringQuery = trpc.engagementIntel.getEngagementScoring.useQuery({ period: '30d' }, { enabled: isAuthenticated });
      ...
      <View style={s.tabContent}>
        ...
        <View style={s.barChart}>
          {scoreDistribution.map((d, i) => {
            const maxCount = Math.max(...scoreDistribution.map(x => x.count), 1);
      ...
      <View style={s.tabContent}>
        <View style={s.serverTrackingCard}>
          ...
          <View style={s.endpointsRow}>
            {serverSideTracking.endpoints.map((ep, i) => (
  ...
  const renderSEO = () => {
    ...
    return (
      <View style={s.tabContent}>
        ...
        {searchKeywords.slice(0, 8).map((kw, i) => (
