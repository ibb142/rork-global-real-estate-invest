...
export default function BroadcastScreen() {
  ...
  const stats = useMemo(() => getBroadcastStats(), []);
  
  const recipients = useMemo(() => {
    if (recipientFilter === 'custom') {
      return selectedRecipients;
    }
    return getBroadcastRecipients(recipientFilter);
  }, [recipientFilter, selectedRecipients]);

  const filteredMembers = useMemo(() => {
    const allRecipients = getBroadcastRecipients('all');
  ...
  const renderComposeTab = () => (
    <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
      <View style={styles.statsRow}>
        ...
        <View style={styles.statCard}>
          <Text style={[styles.statValue, { color: Colors.positive }]}>{stats.deliveryRate}%</Text>
          <Text style={styles.statLabel}>Delivered</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={[styles.statValue, { color: Colors.primary }]}>{stats.openRate}%</Text>
          ...
          <View style={styles.modalContent}>
              ...
              {RECIPIENT_FILTERS.map((filter) => (
                ...
                >
                  ...
                  <Text style={styles.filterCount}>
                    {getBroadcastRecipients(filter.key === 'custom' ? 'all' : filter.key).length}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            >
              <Text style={styles.applyBtnText}>
                Apply ({recipientFilter === 'custom' ? selectedRecipients.length : getBroadcastRecipients(recipientFilter).length} recipients)
              </Text>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  draftSendText: { color: Colors.textSecondary, fontSize: 13 },
