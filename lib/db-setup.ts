...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='analytics_select_own' AND tablename='analytics_events') THEN CREATE POLICY analytics_select_own ON analytics_events FOR SELECT USING (auth.uid()=user_id); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='analytics_insert_auth' AND tablename='analytics_events') THEN CREATE POLICY analytics_insert_auth ON analytics_events FOR INSERT WITH CHECK (auth.uid() IS NOT NULL); END IF;
END $$;
