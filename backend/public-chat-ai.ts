...
import { applySeniorDeveloperNarrativeGate } from './services/ivx-senior-developer-narrative-gate';
...
export async function generatePublicChatAnswer(input: {
  ...
  const gateAnswer = async (result: PublicChatAnswerResult): Promise<PublicChatAnswerResult> => {
    ...
    // SENIOR DEVELOPER NARRATIVE GATE — the chat model cannot read the repo, run
    // tests, commit, or deploy. When the owner asks about patches / dev / QA /
    // deploy / Senior Developer work, OR the model invents a patch narrative
    // ("Workspace Inspection Results", "Recent Patches", fake "Files Changed",
    // "Deploy Authorization Needed"), replace it with real proof (only produced by
    // the live owner-session run) or the strict BLOCKED block. No real proof can
    // originate from this chat path, so it always returns BLOCKED here.
    const seniorDevGate = applySeniorDeveloperNarrativeGate({ message: input.message, answer: result.answer });
    if (seniorDevGate.gated) {
      console.log('[PublicChatAI] Senior Developer narrative gate intervened:', {
        sessionId: input.sessionId,
        routed: seniorDevGate.routed,
        forbiddenMarkers: seniorDevGate.forbiddenMarkers,
      });
      return { ...result, answer: seniorDevGate.answer };
    }
    return result;
  };
