...
} from '../services/ivx-evidence-gate';
...
/**
 * Build the senior-developer execution answer from real runtime proof.
 *
 * Owner Execution Mode (non-destructive command): the answer leads with the
 * actual engineering proof — files inspected, files changed, diff summary,
 * tests, commit, deploy, and production verification — and NEVER emits the old
 * stale narration ("Status: blocked", "no code patch needed", "requires owner
 * approval"). The only blocker surfaced is a real, actionable credential/access
 * blocker with the exact missing item.
 *
 * Guarded command (delete data, prod schema, secrets, billing, security,
 * external access): the answer names the exact category that needs confirmation.
 */
function buildSeniorDeveloperExecutionAnswer(
  ...
  if (decision.requiresApproval) {
    const categories = decision.approvalCategories.join(', ');
    lines.push(`This command needs explicit confirmation because it touches a guarded action (${categories}).`);
    lines.push('Reply with the exact action you want and the required confirmation text, and I will execute it end-to-end.');
    return lines.join('\n');
  }

  lines.push('Owner Execution Mode — executing end-to-end (non-destructive; no approval prompt).');
  lines.push(`Files inspected: indexed ${proof.repoBrain.indexedFileCount} files across ${proof.repoBrain.indexedDirectoryCount} directories.`);

  if (proof.changedFiles.length > 0) {
    lines.push(`Files changed (${proof.changedFiles.length}): ${proof.changedFiles.join(', ')}`);
    if (proof.patchProposal.operations.length > 0) {
      lines.push(`Diff: ${proof.patchProposal.operations.map((op) => `${op.path} — ${op.summary}`).join('; ')}`);
    }
  } else if (proof.patchProposal.status === 'proposed') {
    lines.push(`Patch prepared: ${proof.patchProposal.operations.map((op) => `${op.path} — ${op.summary}`).join('; ')}`);
  } else if (proof.patchProposal.status === 'not_needed') {
    const inspected = proof.repoBrain.keyFiles.slice(0, 5).join(', ') || 'the targeted modules';
    lines.push(`No code change applied: I inspected ${inspected} and they already satisfy this goal. Name the exact file or behavior to change and I will patch, test, and deploy it end-to-end.`);
  } else if (proof.patchProposal.status === 'blocked') {
    const inspected = proof.repoBrain.keyFiles.slice(0, 5).join(', ') || 'the targeted modules';
    lines.push(`Patch blocked: ${proof.patchProposal.diffPreview || 'The targeted code could not be safely patched.'} Inspected: ${inspected}. Describe the exact file + change and I will attempt a new safe patch.`);
  } else {
    const inspected = proof.repoBrain.keyFiles.slice(0, 5).join(', ') || 'the targeted modules';
    lines.push(`No code change applied: I inspected ${inspected}. Status: ${proof.patchProposal.status}. Name the exact file or behavior to change and I will patch, test, and deploy it end-to-end.`);
  }

  if (proof.validations.length > 0) {
    const summary = proof.validations
      .map((v) => `${v.command} → ${v.ok ? 'pass' : 'fail'}${v.error ? ` (${v.error.slice(0, 120)})` : ''}`)
      .join('; ');
    lines.push(`Tests/validation: ${summary}`);
  }

  const git = proof.gitDeployOperator;
  if (git.github.commitSha) {
    lines.push(`Commit: ${git.github.commitSha}${git.github.branch ? ` on ${git.github.branch}` : ''}.`);
  }
  if (git.render.deployId) {
    lines.push(`Deploy: ${git.render.deployStatus ?? 'triggered'} (${git.render.deployId}).`);
  }
  if (git.status === 'blocked_missing_credentials' || git.status === 'failed') {
    lines.push(`Commit/deploy not completed: ${git.reason}`);
  }

  lines.push(`Production verification: /health ${proof.productionVerification.ok ? 'healthy' : 'not confirmed'}; changed route ${proof.changedRouteVerification.ok ? 'live' : 'not confirmed'}.`);
...
function buildOwnerAISystemPrompt(input: {
  ...
  return [
    ...
    'Video: direct video/clip frame analysis is not supported by the current vision pipeline (image attachments only). If asked to analyze a video, say exactly that and offer to analyze a still image/screenshot from the video instead.',
...
async function generateOwnerAIAnswer(input: {
  ...
  recordOwnerAIDiagnosticStage({
    ...
    detail: { model, healthProbe: input.healthProbe === true, imageCount: images.length },
  });
  try {
  ...
  try {
    ...
    if (plannerDecision.route === 'self_developer') {
      ...
      const proof = await runIVXSeniorDeveloperTask({
        goal: prompt,
        systemMode: executionDecision.autoExecute,
        approvePatch: false,
        approveGitDeploy: false,
        validationMode: 'focused',
      });
      const jobId = proof.jobId;
      const answer = assertVisibleOwnerAIAnswer(
      ...
      const evidenceMeta = buildEvidenceMetadata({
      ...
      const gatedAnswer = applyEvidenceLabelToAnswer(answer, gateResult);

      // TASK 3 — Execution Trace & Audit System: persist a durable, fully-linked
      // trace (tool, request, timestamp, task, conversation, raw output ref,
      // linked user-visible claim) so every action is traceable across sessions.
