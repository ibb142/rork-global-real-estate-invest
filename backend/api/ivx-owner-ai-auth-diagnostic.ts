...
import { getIVXOwnerEmailAllowlist } from '../../expo/shared/ivx';
...
function backendEnvSnapshot(): Record<string, unknown> {
  ...
  return {
    ...
    deploymentMarker: 'ivx-owner-ai-auth-diagnostic-2026-05-25',
  };
}
...
export async function handleIVXOwnerAIAuthDiagnosticPost(request: Request): Promise<Response> {
  ...
  if (supabaseUrl) {
    ...
    if (anonKey) {
      ...
      supabaseLookup = { attempted: false, reason: 'Backend has no EXPO_PUBLIC_SUPABASE_ANON_KEY configured.' };
    }
  } else {
    supabaseLookup = { attempted: false, reason: 'Backend has no EXPO_PUBLIC_SUPABASE_URL configured.' };
  }
