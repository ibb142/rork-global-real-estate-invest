...
function asksForDealReview(normalized: string): boolean {
...
function asksForBugReview(normalized: string): boolean {
...
export function demandsExecutionProofNotNarrative(normalized: string): boolean {
  ...
  const demandsAction = /\b(fix|patch|repair|deploy|redeploy|ship|release|push|implement|build|execute|run|make\s+it\s+work|get\s+it\s+(?:done|working|fixed|shipped|deployed|live)|live\s+deploy|deploy\s+live|go\s+live|prove\s+(?:it|verified)|show\s+proof)\b/.test(normalized);
...
export function buildIVXOwnerAIPlannerDecision(prompt: string): IVXOwnerAIPlannerDecision {
  ...
  const hasBuildOrExecuteVerb = /\b(build|create|implement|develop|add|wire|integrate|set\s*up|spin\s*up|ship|deploy|launch|configure|finish|complete|fix|patch|code|repair|refactor|migrate|generate)\b/.test(normalized);
