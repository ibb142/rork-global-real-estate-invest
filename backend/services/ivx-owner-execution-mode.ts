...
const APPROVAL_GATES: Array<{ category: OwnerApprovalCategory; label: string; pattern: RegExp }> = [
  {
    ...
    label: 'deletes data',
    pattern:
      /\b(delet(?:e|ing)|drop(?:ping)?|truncat(?:e|ing)|wip(?:e|ing)|purg(?:e|ing)|eras(?:e|ing)|remov(?:e|ing)\s+(?:all|the)\s+(?:data|rows?|records?|users?|tables?))\b.{0,40}\b(data|rows?|records?|table|tables|database|db|users?|accounts?|bucket|storage|production|prod)\b|\b(drop\s+table|truncate\s+table|delete\s+from|rm\s+-rf)\b/i,
  },
  {
