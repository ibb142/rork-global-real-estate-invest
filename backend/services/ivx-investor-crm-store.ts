...
/** Create a new investor record. Returns null with the validation error on failure. */
export async function createInvestor(
  input: CreateInvestorInput,
): Promise<{ ok: true; investor: InvestorRecord } | { ok: false; error: string }> {
  ...
  await appendEvent({ type: 'create', investor: record, at: record.createdAt });
