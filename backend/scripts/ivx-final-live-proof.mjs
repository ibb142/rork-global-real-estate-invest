...
async function main() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const backendCommitMatch = backendVersion.body?.commit === EXPECTED_SHA || backendVersion.body?.commit?.startsWith(SHORT_SHA) || backendDeploy?.commit === EXPECTED_SHA || backendHealth.body?.commit === EXPECTED_SHA || backendHealth.body?.commit?.startsWith(SHORT_SHA);
    ...
    const poll = {
      ...
      backendHealth: { status: backendHealth.status, sha: backendHealth.body?.commit, commitMatch: backendCommitMatch },
